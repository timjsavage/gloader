...
template "#{node['apache']['dir']}/sites-available/kibana" do
  ...
  variables(:docroot => "#{node[:logstash][:basedir]}/kibana/current",
            :server_name => "#{node[:logstash][:kibana][:apache2_server_name]}")
  notifies :restart, "service[apache2]"
end

template "#{node[:logstash][:basedir]}/kibana/htpasswd" do
  variables( :username => node.logstash.kibana.basic_auth_username,
             :password => node.logstash.kibana.basic_auth_password)
